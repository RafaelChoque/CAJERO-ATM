...
import { Server, MapPin, Plus, X, Search, Edit2, Map, Maximize, Minimize, ChevronDown, ChevronUp, Upload, AlertTriangle, PackageOpen } from 'lucide-react';
...
const MapResizer = ({ isExpanded }) => {
    const map = useMap();
    useEffect(() => { setTimeout(() => { map.invalidateSize(); }, 300); }, [map, isExpanded]);
    return null;
};
...
const VistaCajeros = () => {
    ...
    const openEditModal = (cajero) => {
        setIsEditing(true);
        setFormData({ ...cajero, latitud: cajero.latitud || '', longitud: cajero.longitud || '' });
    ...
    const handleMapClick = (pos) => {
        setMapPosition(pos);
        setFormData({ ...formData, latitud: pos.lat.toString(), longitud: pos.lng.toString() });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await apiCall(url, { method: method, body: JSON.stringify(formData) });
            if (!res.ok) throw new Error("Error al procesar la información del ATM.");
            showSuccess(isEditing ? 'ATM Actualizado' : 'ATM Registrado', isEditing ? `Los datos de <b>${formData.codigoCajero}</b> se guardaron.` : `La terminal <b>${formData.codigoCajero}</b> fue vinculada.`);
            setShowModal(false);
            setFormData(initialFormState);
            cargarCajeros();
        } catch (error) {
        ...
        try {
            ...
            if (responseAlertas.ok) {
                ...
                dataAlertas.forEach(alerta => {
                    diccionarioAlertas[alerta.idCaseta] = alerta;
                });
                setAlertasStock(prev => ({ ...prev, [idCajero]: diccionarioAlertas }));
            }

        } catch (error) {
            showError("No se pudieron cargar los detalles del cajero.");
        } finally {
    ...
    const subirExcelCaseta = async (idCaseta, file) => {
        ...
        try {
            ...
            const response = await apiCall(`/api/admin/casetas/${idCaseta}/cargar-billetes`, { method: "POST", body: formData });
            if (!response.ok) {
                const data = await response.json();
                throw new Error(data.message || "No se pudo cargar el Excel");
            }
            showSuccess("Carga exitosa", "Los billetes fueron cargados correctamente en la caseta.");
            recargarDatosCajeroExpandido();
        } catch (error) {
            showError(error.message || "No se pudo cargar el archivo Excel.");
        }
    };
    ...
    const subirRemesaMaestra = async (idCajero, file) => {
        ...
        try {
            const formData = new FormData();
            formData.append("archivo", file);
            const response = await apiCall(`/api/admin/casetas/cajero/${idCajero}/cargar-remesa`, { method: "POST", body: formData });
            if (!response.ok) {
                const data = await response.json();
                throw new Error(data.message || "No se pudo cargar la remesa maestra.");
            }
            showSuccess("Bóveda Llenada", "La remesa fue procesada y distribuida en todas las casetas del cajero automáticamente.");
            recargarDatosCajeroExpandido();
        } catch (error) {
            showError(error.message || "Error al procesar el Excel Maestro.");
        }
    };

    const recargarDatosCajeroExpandido = async () => {
        if (expandedCajeroId) {
            setExpandedCajeroId(null);
    ...
    const getEstadoCasetaColor = (estado) => {
        switch (estado) {
            case 'ACTIVA': return 'bg-green-50 text-green-600 border-green-200';
            case 'MANTENIMIENTO': return 'bg-yellow-50 text-yellow-600 border-yellow-200';
            case 'BLOQUEADA': return 'bg-red-50 text-red-600 border-red-200';
            ...
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                    <div className="relative max-w-md">
                        ...
                        <input type="text" className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#f5d000] outline-none transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Buscar cajero o ubicación..." />
                    </div>
                </div>

                {/* tabla principal de cajeros */}
                                    ...
                                    <React.Fragment key={cajero.idCajero}>
                                        <tr className="hover:bg-blue-50/30 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-sm text-gray-600 font-medium">
                                                <MapPin size={14} className="inline mr-1 text-[#004a8e]" />
                                                {cajero.ubicacion}
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center justify-end gap-3">
                                                    <button onClick={() => toggleCasetas(cajero.idCajero)} className="p-1.5 text-purple-600 bg-purple-50 hover:bg-purple-600 hover:text-white rounded-md transition-all shadow-sm" title="Ver casetas">
                                                        {expandedCajeroId === cajero.idCajero ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                                                    </button>
                                                    ...
                                                    <button onClick={() => toggleEstado(cajero.idCajero, cajero.estado, cajero.codigoCajero)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#004a8e] focus:ring-offset-1 ${cajero.estado === 'ACTIVO' ? 'bg-green-500' : 'bg-gray-300'}`}>
                                                ...
                                                <td colSpan="4" className="bg-gray-50 px-6 py-4">
                                                    <div className="rounded-xl border border-gray-200 bg-white overflow-hidden shadow-inner">
                                                        ...
                                                        <div className="px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white flex justify-between items-center">
                                                            <div>
                                                                <h3 className="text-sm font-black text-[#004a8e] uppercase tracking-wide flex items-center gap-2">
                                                                    <Server size={18} /> Casetas de {cajero.codigoCajero}
                                                                </h3>
                                                                <p className="text-xs text-gray-500 mt-1">Gestión de efectivo y control de bóveda.</p>
                                                            </div>
                                                            ...
                                                            <div>
                                                                <input
                                                                    ...
                                                                    accept=".xlsx,.xls"
                                                                    style={{ display: 'none' }}
                                                                    ref={(el) => (fileInputMasterRef.current[cajero.idCajero] = el)}
                                                                    onChange={(e) => {
                                                                        subirRemesaMaestra(cajero.idCajero, e.target.files[0]);
                                                                        e.target.value = '';
                                                                    }}
                                                                />
                                                                <button
                                                                    onClick={() => abrirSelectorMaestro(cajero.idCajero)}
                                                                    className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg active:scale-95 transition-all"
                                                                >
                                                                    <PackageOpen size={18} /> Cargar Remesa Completa
                                                                </button>
                                                            </div>
                                                        </div>

                                                        {cargandoCasetas && !casetasPorCajero[cajero.idCajero] ? (
                                                            <div className="p-6 text-sm text-gray-500 font-medium text-center">Analizando bóveda...</div>
                                                        ) : !casetasPorCajero[cajero.idCajero] || casetasPorCajero[cajero.idCajero].length === 0 ? (
                                                            <div className="p-6 text-sm text-gray-500 font-medium text-center">Este cajero no tiene casetas registradas.</div>
                                                        ) : (
                                                            <div className="overflow-x-auto">
                                                                <table className="w-full text-sm">
                                                                    <thead className="bg-white border-b border-gray-100">
                                                                        <tr>
                                                                            <th className="px-5 py-3 text-left text-xs font-bold text-[#004a8e] uppercase">Corte</th>
                                                                            <th className="px-5 py-3 text-center text-xs font-bold text-[#004a8e] uppercase">Stock Actual</th>
                                                                            <th className="px-5 py-3 text-left text-xs font-bold text-[#004a8e] uppercase">Estado</th>
                                                                            <th className="px-5 py-3 text-right text-xs font-bold text-[#004a8e] uppercase">Carga Individual</th>
                                                                        </tr>
                                                                    </thead>
                                                                    <tbody className="divide-y divide-gray-50">
                                                                        {casetasPorCajero[cajero.idCajero].map((caseta) => {
                                                                            ...
                                                                            return (
                                                                            <tr key={caseta.idCaseta} className="hover:bg-gray-50/50">
                                                                                <td className="px-5 py-4 font-black text-gray-800 text-base">
                                                                                    Bs {caseta.denominacion}
                                                                                </td>
                                                                                <td className="px-5 py-4 text-center">
                                                                                    <div className="flex flex-col items-center">
                                                                                        <span className={`text-lg font-black ${alerta ? 'text-red-600' : 'text-gray-700'}`}>
                                                                                            {caseta.stockActual} / {caseta.capacidadMaxima}
                                                                                        </span>

                                                                                        {alerta && (
                                                                                            <div className="mt-2 bg-red-50 border border-red-200 text-red-700 px-3 py-2 rounded-lg text-xs font-medium text-left flex items-start gap-2 max-w-xs shadow-sm">
                                                                                                <AlertTriangle size={16} className="shrink-0 mt-0.5 text-red-500" />
                                                                                                <div>
                                                                                                    <p className="font-bold mb-1">¡Nivel Crítico de Efectivo!</p>
                                                                                                    <p>El modelo EOQ sugiere recargar exactamente <b>{alerta.cantidadSugeridaEOQ} billetes</b> para minimizar costos logísticos.</p>
                                                                                                </div>
                                                                                            </div>
                                                                                        )}
                                                                                    </div>
                                                                                ...
                                                                                <td className="px-5 py-4">
                                                                                    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase border ${getEstadoCasetaColor(caseta.estado)}`}>
                                                                                        {caseta.estado}
                                                                                    </span>
                                                                                </td>
                                                                                <td className="px-5 py-4 text-right">
                                                                                    <input
                                                                                        ...
                                                                                        accept=".xlsx,.xls"
                                                                                        style={{ display: 'none' }}
                                                                                        ref={(el) => (fileInputsRef.current[caseta.idCaseta] = el)}
                                                                                        onChange={(e) => {
                                                                                            subirExcelCaseta(caseta.idCaseta, e.target.files[0]);
                                                                                            e.target.value = '';
                                                                                        }}
                                                                                    />
                                                                                    <button
                                                                                        onClick={() => abrirSelectorArchivo(caseta.idCaseta)}
                                                                                        className="inline-flex items-center gap-2 bg-white border-2 border-[#004a8e] text-[#004a8e] hover:bg-blue-50 px-4 py-2 rounded-lg font-bold text-xs uppercase tracking-wide transition-all active:scale-95"
                                                                                    >
                    ...
                    <div className={`bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row animate-in zoom-in-95 duration-200 transition-all ${isMapExpanded ? 'w-full h-full max-w-none' : 'w-full max-w-5xl'}`}>
                        <div className={`w-full md:w-5/12 p-6 flex flex-col bg-gray-50 border-r border-gray-100 ${isMapExpanded ? 'hidden' : 'flex'}`}>
                            <div className="flex justify-between items-center mb-6">
                                <div>
                                    <h3 className="text-lg font-black text-[#004a8e] uppercase tracking-tight">{isEditing ? 'Configurar ATM' : 'Nueva Terminal'}</h3>
                                    <p className="text-xs text-gray-500 font-medium">Complete los datos y marque el mapa</p>
                                </div>
                                <button onClick={() => setShowModal(false)} className="md:hidden text-gray-400 hover:bg-gray-200 hover:text-gray-900 rounded-lg p-1.5 transition-colors"><X size={20} /></button>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="flex-1 space-y-4">
                                ...
                                <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 mt-6">
                                    ...
                                    <button type="submit" className="px-5 py-2.5 bg-[#004a8e] text-white rounded-xl font-black uppercase hover:bg-[#003566] transition-colors shadow-md">{isEditing ? 'Guardar Cambios' : 'Vincular ATM'}</button>
                                </div>
                            </form>
                        </div>

                        <div className={`relative bg-gray-200 ${isMapExpanded ? 'w-full h-full' : 'w-full md:w-7/12 min-h-[450px]'}`}>
                            {!isMapExpanded && <button onClick={() => setShowModal(false)} className="hidden md:flex absolute top-4 right-4 z-[400] bg-white text-gray-500 hover:text-red-500 hover:bg-red-50 rounded-full p-2 shadow-lg transition-colors items-center justify-center"><X size={20} /></button>}
                            <button type="button" onClick={(e) => { e.preventDefault(); setIsMapExpanded(!isMapExpanded); }} className="absolute top-4 left-4 z-[400] bg-white/90 backdrop-blur px-4 py-2 rounded-xl shadow-lg border border-blue-200 flex items-center gap-2 text-xs font-bold text-[#004a8e] hover:bg-blue-50 transition-colors cursor-pointer">
                                {isMapExpanded ? <Minimize size={16} /> : <Maximize size={16} />} {isMapExpanded ? 'Minimizar Mapa' : 'Pantalla Completa'}
                            </button>
